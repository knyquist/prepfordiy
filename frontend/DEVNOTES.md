# The following is from ngbp, the angular scaffold used in this project: http://github.com/ngbp/ngbp

# The Tools Used in `redditDIY`
...
`redditDIY` uses git to manage its codebase. While in theory you don't have
to use Git once you download `redditDIY`, this project makes the assumption
...
While `redditDIY` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `redditDIY` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `redditDIY`; Grunt is
...
to get started with `redditDIY`. But for those curious or looking to go a
...
Grunt is the engine behind `redditDIY`. It's the magic that makes it move.
...
`redditDIY` comes with a `bower.json` file that looks something like this:
...
in `redditDIY` by default in there to see what I mean.
